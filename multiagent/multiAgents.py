...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        #tinh count dua tren khoang cach toi food
        #food cang gan thi trang thai cang tot
        ...
        #tinh khoang cach toi moi ghost
        for ghost in successorGameState.getGhostPositions():
            ghostDistances.append(manhattanDistance(ghost,newPos))

        #tinh count dua tren khoang cach toi ghost
        #ghost cang gan thi trang thai cang xau
        for ghost in successorGameState.getGhostPositions():
            if ghost == newPos:
                count = 2 - count
...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def miniMax(gameState,agent,depth):
            ...
            # tat ca ghost hoan thanh 1 vong: tang depth
            ...
            # ghost chua het: chon ghost tiep theo
            else:
                nextAgent = agent + 1

            # voi moi trang thai tiep theo, tim gia tri minimax
            for action in gameState.getLegalActions(agent):

                if not result: # nuoc dau tien
                    nextValue = miniMax(gameState.generateSuccessor(agent,action),nextAgent,depth)

                    # gan gia tri minimax va action cho ket qua
                    result.append(nextValue[0])
                    result.append(action)
                else:
                    # kiem tra gia tri minimax co tot hon gia tri truoc khong
        ...
        # goi ham miniMax voi depth = 0, va lay 1 action
        # pacman choi truoc => agent = self.index == 0
