    ...
    while not path.isEmpty():
        ...
        if currentState not in visited:
            ...
            for nextState, action, cost in problem.getSuccessors(currentState):
                ...
                path.push((nextState, nextActions))
    util.raiseNotDefined()
...
def uniformCostSearch(problem):
    ...
    while not path.isEmpty():
        currentState, actions, lastCost = path.pop()
        if currentState not in visited:
            visited = visited + [currentState]
            if problem.isGoalState(currentState):
                return actions
            for nextState, action, cost in problem.getSuccessors(currentState):
                nextActions = actions + [action]
                nextCost = lastCost + cost
                path.push((nextState, nextActions, nextCost), nextCost)
    util.raiseNotDefined()
    ...
    while not path.isEmpty():
        ...
        if currentState not in visited:
            ...
            for nextState, action, cost in problem.getSuccessors(currentState):
                nextActions = actions + [action]
                nextCost = lastCost + cost
                nextHeuristic = nextCost + heuristic(nextState, problem)
